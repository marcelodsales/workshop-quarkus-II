...
wait_for_http() {
  local url=$1
  for _ in {1..60}; do
    if curl -s -o /dev/null "$url" >/dev/null 2>&1; then return 0; fi
    sleep 1
  done
...
sleep 10
jcmd "$SPRING_PID" GC.run >/dev/null 2>&1
sleep 3
SPRING_RSS=$(ps -p "$SPRING_PID" -o rss= | awk '{printf "%.2f", $1/1024}')
SPRING_HEAP=$(jcmd "$SPRING_PID" GC.heap_info 2>/dev/null | awk '/garbage-first heap/ {for(i=1;i<=NF;i++) if($i=="used") {gsub(/K/,"",$(i+1)); printf "%.2f", $(i+1)/1024; exit}}')
[ -z "$SPRING_HEAP" ] && SPRING_HEAP="-"
...
podman run -d --rm --name "$SPRING_CONTAINER" --memory 256m --cpus 1 -p 8080:8080 -e SPRING_DATASOURCE_URL=jdbc:postgresql://host.containers.internal:5432/banking spring-banking:latest >/dev/null
...
SPRING_AB_OUTPUT=$(ab -n 20000 -c 50 http://localhost:8080/accounts/$SPRING_ACCOUNT_ID/balance 2>&1)
...
sleep 10
jcmd "$QUARKUS_PID" GC.run >/dev/null 2>&1
sleep 3
QUARKUS_RSS=$(ps -p "$QUARKUS_PID" -o rss= | awk '{printf "%.2f", $1/1024}')
QUARKUS_HEAP=$(jcmd "$QUARKUS_PID" GC.heap_info 2>/dev/null | awk '/garbage-first heap/ {for(i=1;i<=NF;i++) if($i=="used") {gsub(/K/,"",$(i+1)); printf "%.2f", $(i+1)/1024; exit}}')
[ -z "$QUARKUS_HEAP" ] && QUARKUS_HEAP="-"
...
podman run -d --rm --name "$QUARKUS_CONTAINER" --memory 256m --cpus 1 -p 8080:8080 -e QUARKUS_DATASOURCE_JDBC_URL=jdbc:postgresql://host.containers.internal:5432/banking quarkus-banking:latest >/dev/null
...
QUARKUS_AB_OUTPUT=$(ab -n 20000 -c 50 http://localhost:8080/accounts/$QUARKUS_ACCOUNT_ID/balance 2>&1)
...
printf "%-20s %-15s %-15s %-15s\n" "RSS (MB)" "$SPRING_RSS" "$QUARKUS_RSS" "$(calc_percent "$SPRING_RSS" "$QUARKUS_RSS")"
if [[ "$QUARKUS_HEAP" != "-" ]]; then
  HEAP_GAIN=$(calc_percent "$SPRING_HEAP" "$QUARKUS_HEAP")
else
  HEAP_GAIN="-"
fi
printf "%-20s %-15s %-15s %-15s\n" "Heap (MB)" "$SPRING_HEAP" "$QUARKUS_HEAP" "$HEAP_GAIN"
